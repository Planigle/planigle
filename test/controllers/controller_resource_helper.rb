module ControllerResourceHelper
  ...
  def test_show_unauthorized
    get :show, params: {:id => 1}.merge(context)
    assert_redirected_to :controller => 'sessions', :action => 'new'
  end
  ...
  def test_show_not_found
    login_as(individuals(:quentin))
    get :show, params: {:id => 999}.merge(context)
    assert_response 404
  end
  ...
  def test_update_not_found
    login_as(individuals(:quentin))
    put :update, params: {:id => 999}.merge(update_success_parameters)
    assert_response 404
  end
  ...
  def test_destroy_not_found
    login_as(individuals(:quentin))
    delete :destroy, params: {:id => 999}.merge(context)
    assert_response 404
  end
