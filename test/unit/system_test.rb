...
class SystemTest < ActiveSupport::TestCase
  ...
  def test_summarize
    ...
    System.summarize
    assert_equal 5, IterationTotal.find(:all, :conditions => {:iteration_id => iteration1.id, :team_id => nil})[0].done
    assert_equal 6, IterationTotal.find(:all, :conditions => {:iteration_id => iteration2.id, :team_id => nil})[0].done
