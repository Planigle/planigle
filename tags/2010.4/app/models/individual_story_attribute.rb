class IndividualStoryAttribute < ActiveRecord::Base
  ...
  acts_as_audited :except => [:individual_id, :story_attribute_id]
  ...
  def initialize_defaults
    if !self.ordering
      highest = IndividualStoryAttribute.find(:first, :joins => :story_attribute, :conditions => ['story_attributes.project_id=? and individual_id=?', story_attribute.project_id, individual_id], :order=>'ordering desc')
