module IuiHelper
  ...
  # WRB - Added default value for initial_caption.
  ...
  # WRB - Changed onclick_two logic (was SoupsOnline).
  def list_element(item, target=nil)
    onclick_one = "$('header_text').innerHTML='#{item.name}'; "
    onclick_two = "$('backButton').addEventListener('click', 
        function() {$('header_text').innerHTML='Planigle'; }, false);" # WRB - Changed to Planigle
    if item.url # WRB - Added
      link = link_to_target(target, item.caption, item.url, 
          :onclick => "#{onclick_one} " + " #{onclick_two}")
      content_tag(:li, link)
    else
      content_tag(:li, item.caption)
    end
  end
