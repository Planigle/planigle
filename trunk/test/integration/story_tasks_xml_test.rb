...
class StoryTasksXmlTest < ActionController::IntegrationTest
  ...
  def test_set_owner_success
    put '/stories/1/tasks/1', {:record => {:individual_id => 2}}, authorization_header
  ...
  def test_set_owner_success_flex
    flex_login
    put '/stories/1/tasks/1.xml', {:record => {:individual_id => 2}}, flex_header
    assert_response :success
    assert_equal tasks(:one).reload.individual_id, 2
  end
  ...
  def test_set_owner_failure
    put '/stories/1/tasks/1', {:record => {:individual_id => 999}}, authorization_header
  ...
  def test_set_owner_failure_flex
    flex_login
    put '/stories/1/tasks/1.xml', {:record => {:individual_id => 999}}, flex_header
    assert_response :success
    assert_select 'errors' do
      assert_select 'error'
    end
