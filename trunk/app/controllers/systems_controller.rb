...
protected
  ...
  def data
    ...
    report_data['iteration_breakdowns'] = Iteration.find(:all, :include => [{:stories => :story_values},:project], :conditions => {:project_id => project_id}).inject([]) {|collect, iteration| collect.concat(CategoryTotal.summarize_for(iteration))}
